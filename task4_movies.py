...
#i) 1. Find N moves with the highest IMD Rating
...
#i) 2. Find N moves with the highest IMD Rating in a given year
def highest_rated_movies_yearly(limit, target_year):
    match_stage = {"$match": {"year.$numberInt":target_year}}
...
# ii) 1. Find top `N` directors -who created the maximum number of movies in a given year
...
#iii) 1. Find top `N` actors - who starred in the maximum number of movies for a given genre
...
# Fetch movies with the highest IMDB ratings.
def fetch_highest_rated_movies(limit):
    result = db.movies.find({}, {"title": 1, "imdb.rating": 1, "_id": 0}).sort("imdb.rating", -1).limit(limit)
    movies_list = [(movie["title"], movie["imdb"]["rating"]) for movie in result]
    return movies_list
...
for title, rating in movies_list:
    print(f"{title}: {rating.get("$numberInt")}")
...
def highest_rated_movies_yearly(limit, target_year):
    ...
    pipeline = [match_stage, project_stage, sort_stage, {"$limit": limit}]
    movies = db.movies.aggregate(pipeline)
    return [(movie["title"], movie["imdb"]["rating"]["$numberDouble"]) for movie in movies]
...
# Aggregate and list actors with the most roles.
def actors_most_roles(limit):
    pipeline = [
        {"$unwind": "$cast"},
        {"$group": {"_id": "$cast", "totalRoles": {"$sum": 1}}},
...
def actors_most_roles_year(limit, year):
    # Match by year, then follow the same logic for counting roles.
    pipeline = [
        {"$match": {"year.$numberInt": year}},
        {"$unwind": "$cast"},
        {"$group": {"_id": "$cast", "totalRoles": {"$sum": 1}}},
...
for genre_info in genre_movies:
    ...
    for movie in genre_info['topMovies']:
        try:
            print(f" - {movie['title']}: {movie['rating']['$numberInt']}")
